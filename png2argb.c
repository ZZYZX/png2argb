...
void process_file()
{
    ...
    if (file_color_type != PNG_COLOR_TYPE_RGB
        ...
        __err("Color_type of input file must be PNG_COLOR_TYPE_RGB (%d)"
...
void read_png_file(char *file_name)
{
    ...
    if (!fp) {
        __err("File %s could not be opened for reading", file_name);
        return;
    }

    fread(header, 1, 8, fp);
    if (png_sig_cmp(header, 0, 8)) {
        __err("File %s is not recognized as a PNG file", file_name);
    }
    ...
    if (!png_ptr) {
        __err("png_create_read_struct failed");
    }

    info_ptr = png_create_info_struct(png_ptr);
    if (!info_ptr) {
        __err("png_create_info_struct failed");
    }

    if (setjmp(png_jmpbuf(png_ptr))) {
        __err("Error during init_io");
    }
...
int main(int argc, char **argv)
{
    ...
    if (isatty(fileno(stdin)) && argc < 2) { // NULL stdin, no arguments
        __err("\n  Usage: %s image.png [image2.png] > buffer.argb\n"
